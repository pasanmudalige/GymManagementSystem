...
public class Validation {
    ...
    public static void phoneNumber(JTextField textFild) {
        ...
        public static void memberId(JTextField textFild) {
        String txt = textFild.getText();
        int caretPosition = textFild.getCaretPosition();
        if (!txt.matches("[A-Za-z//[0-9] ]{0,6}$")) {
            txt = txt.substring(0, caretPosition - 1) + txt.substring(caretPosition);
            textFild.setText(txt);
            textFild.setCaretPosition(caretPosition - 1);
        }
 ...
 public static void priceTextA(JTextField textField) {
    ...
    public static void priceText(JTextField textField) {
        String text = textField.getText();
        if (!text.isEmpty()) {
            boolean res = text.matches("[0-9]{0,2}+[.]?[0-9]{0,2}");
            if (!res) {
                textField.setText(text.substring(0, text.length() - 1));
            }
        }
    ...
    public static void priceTextAdvance(JTextField textField) {
        String text = textField.getText();
        if (!text.isEmpty()) {
            boolean res = text.matches("[0-9]+[.]?[0-9]{0,3}");
            if (!res) {
                textField.setText(text.substring(0, text.length() - 1));
            }
        }
